...
def main():
    ...
    model = GAMENet(voc_size, ehr_adj, ddi_adj, emb_dim=153, device=device, ddi_in_memory=DDI_IN_MEM)
    # if TEST:
    ...
    if TEST:
        ...
        print(history['ddi_rate'])
        plt.figure()
        plt.plot(epoch_count, history['ddi_rate'], label='DDI Rate')
        # plt.plot(epoch_count, history['ja'], label='Jaccard')
        # plt.plot(epoch_count, history['avg_p'], label='AVG_PRC')
        # plt.plot(epoch_count, history['avg_r'], label='AVG_RECALL')
        # plt.plot(epoch_count, history['avg_f1'], label='AVG_F1')
        # plt.plot(epoch_count, history['prauc'], label='PRAUC')

        plt.title('DDI_Rate')
        plt.xlabel('Epochs')
        plt.ylabel('DDI_Rate')
        plt.legend()
        plt.savefig('ddi_rate.png')
