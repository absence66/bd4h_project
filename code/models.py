...
class GCN(nn.Module):
    def __init__(self, voc_size, emb_dim, adj, device=torch.device('cuda:0')):
        ...
        self.voc = torch.eye(voc_size).to(device)

        self.gcn1 = GraphConvolution(voc_size, adj.shape[0])
        self.dropout = nn.Dropout(p=0.3)
        self.gcn2 = GraphConvolution(adj.shape[0], adj.shape[0])
    

    def forward(self):
        node_embedding = self.gcn1(self.voc, self.adj)
...
class GAMENet(nn.Module):
    def __init__(self, vocab_size, ehr_adj, ddi_adj, emb_dim=153, device=torch.device('cuda:0'), ddi_in_memory=True):
        super(GAMENet, self).__init__()
        K = len(vocab_size)
        self.K = K
        self.vocab_size = vocab_size
        self.device = device
        self.tensor_ddi_adj = torch.FloatTensor(ddi_adj)
